    ...
    function automatic [AHB_DATA_W-1:0] calc_read_mask(input [2:0] size);
        begin
            case (size[1:0])
                ...
                2'b01:
                calc_read_mask = {{(AHB_DATA_W - 16) {1'b0}}, 16'hFFFF};  // Halfword: 0x0000FFFF
    ...
    function automatic [(EXT_DATA_W/8)-1:0] calc_byteenable;
        ...
        begin
            ...
            for (i = 0; i < total_bytes; i = i + 1) begin
                if ((i >= byte_index) && (i < byte_index + bs)) calc_byteenable[i] = 1'b1;
            end
        end
    endfunction
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (DEBUG && (state != next_state)) begin
                $display("[%0t] [AHB_BRIDGE] State transition: %s -> %s", $time, get_state_name(
                         state), get_state_name(next_state));
            end
            ...
            if ((state == IDLE || state == RESP) && hreadyout) begin
                if (hsel && trans_valid) begin
                    ...
                    if (DEBUG) begin
                        $display(
                            "[%0t] [AHB_BRIDGE] Address phase captured: addr=0x%0h, %s, size=%s, htrans=%b, hreadyout=%b",
                            $time, haddr, hwrite ? "WRITE" : "READ", get_size_name(hsize), htrans,
                            hreadyout);
                    end
                end else begin
            ...
            if (state == ADDR) begin
                ...
                if (DEBUG) begin
                    $display(
                        "[%0t] [AHB_BRIDGE] Data phase captured: hwdata=0x%0h (stored in wdata_reg)",
                        $time, hwdata);
                end
            end
            ...
            case (state)
                ...
                ADDR: begin
                    ...
                    if (write_reg) begin
                        ...
                        if (DEBUG) begin
                            $display(
                                "[%0t] [AHB_BRIDGE] EXT_WRITE: ext_addr=0x%0h, ext_wdata=0x%0h, byteen=0x%0h, byte_offset=%0d",
                                $time, addr_reg[AHB_ADDR_W-1:EXT_ADDR_OFFSET],
                                (hwdata << (byte_offset * 8)), calc_byteenable(addr_reg, size_reg),
                                byte_offset);
                        ...
                        if (DEBUG) begin
                            $display(
                                "[%0t] [AHB_BRIDGE] EXT_READ: ext_addr=0x%0h, byteen=0x%0h, byte_offset=%0d",
                                $time, addr_reg[AHB_ADDR_W-1:EXT_ADDR_OFFSET], calc_byteenable(
                                addr_reg, size_reg), byte_offset);
                            $display("[%0t] [AHB_BRIDGE]   AHB: addr=0x%0h, size=%s", $time,
                                     addr_reg, get_size_name(size_reg));
                        end
                    end
                    // hreadyout is driven by hreadyout_next combinational logic
                end
                WAIT_ACK: begin
                    ...
                    if (DEBUG && !ext_ack) begin
                        $display(
                            "[%0t] [AHB_BRIDGE] WAIT_ACK: Waiting for ext_ack (ext_read=%b, ext_write=%b)",
                    ...
                    if (ext_ack) begin
                        ...
                        if (!write_reg) begin
                            ...
                            if (DEBUG) begin
                                $display(
                                    "[%0t] [AHB_BRIDGE] EXT_ACK received (WRITE): Write completed",
                                    $time);
                            end
                        end
                        ...
                        if (DEBUG) begin
                            ...
                            if (wait_cycles > 10) begin
                                $display(
                                    "[%0t] [AHB_BRIDGE] WARNING: Waiting for ext_ack for %0d cycles!",
                                    $time, wait_cycles);
                                wait_cycles <= 0;
                            end
                            ...
                            if (stuck_count > 20) begin
                                $display(
                                    "[%0t] [AHB_BRIDGE] ERROR: Stuck in WAIT_ACK for %0d cycles! ext_ack=%b, ext_read=%b, ext_write=%b",
                ...
                RESP: begin
                    ...
                    if (DEBUG) begin
                        if (!write_reg) begin
                            $display(
                                "[%0t] [AHB_BRIDGE] RESP: Presenting hrdata=0x%0h to AHB master",
                                $time, hrdata);
                        end else begin
        ...
        if (DEBUG) begin
            always @(posedge clk) begin
                ...
                if (hsel && !hreadyout && (htrans == 2'b10 || htrans == 2'b11)) begin
                    $display(
                        "[%0t] [AHB_BRIDGE] WARNING: AHB transaction active but hreadyout=0 (state=%s)",
                ...
                if (ext_ack && !ext_read && !ext_write) begin
                    $display(
                        "[%0t] [AHB_BRIDGE] ERROR: ext_ack asserted without ext_read or ext_write! (state=%s)",
                        $time, get_state_name(state));
                end
                ...
                if (ext_read && ext_write) begin
                    $display(
                        "[%0t] [AHB_BRIDGE] ERROR: Both ext_read and ext_write asserted simultaneously! (state=%s)",
                        $time, get_state_name(state));
                end
                ...
                if (state == ADDR) begin
                    if (addr_reg[EXT_ADDR_OFFSET-1:0] != 0 && size_reg >= 2'b10) begin
                        $display(
                            "[%0t] [AHB_BRIDGE] WARNING: Word/DWord access to unaligned address 0x%0h (offset=%0d)",
